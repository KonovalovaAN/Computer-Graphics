...
RasterAlgorithms::RasterAlgorithms(QWidget *parent) : QWidget{parent}
{
    ...
    timeLbl = new QLabel("Время выполнения:");
    ...
    algorithmsLay->addWidget(timeLbl);
...
void RasterAlgorithms::lineData() { lineOrCircleDataVisible(true); }

void RasterAlgorithms::circleData() { lineOrCircleDataVisible(false); }
...
void RasterAlgorithms::goToGraphic()
{
    ...
    else
    {
        ...
        wgt->xAxis->setRange(lastX0 - x, lastX1 + x);
        wgt->yAxis->setRange(lastY0 - y, lastY1 + y);
    }
...
void RasterAlgorithms::drawPlot()
{
    ...
    if (bresenhamCircleRadBt->isChecked())
    {
        ...
        lastY0 = y0;
        lastR = rad;
        elapsedTime = bresenhamCircleAlgorithm(x0, y0, rad);
    }
    else
    {
        lastX0 = std::min(x0, x1);
        lastY0 = std::min(y0, y1);
        lastX1 = std::max(x0, x1);
        lastY1 = std::max(y0, y1);
        lastR = -1;

        if (stepByStepRadBt->isChecked())
            elapsedTime = stepAlgorithm(x0, y0, x1, y1);
        else if (cdaRadBt->isChecked())
            elapsedTime = ddaAlgorithm(x0, y0, x1, y1);
        else if (bresenhamLineRadBt->isChecked())
            elapsedTime = bresenhamLineAlgorithm(x0, y0, x1, y1);
        else
            elapsedTime = castlePitwayAlgorithm(x0, y0, x1, y1);
    }

    timeLbl->setText("Время выполнения: " + QString::number(elapsedTime) + " мс");
    wgt->replot();
}

qint64 RasterAlgorithms::stepAlgorithm(int x0, int y0, int x1, int y1)
{
    QElapsedTimer timer;
        timer.start();
    if (x0 > x1)
...
qint64 RasterAlgorithms::ddaAlgorithm(int x0, int y0, int x1, int y1)
{
    QElapsedTimer timer;
        timer.start();
    if (x0 > x1)
    ...
    while (i < l)
    {
        y += dy/l;
        x += dx/l;
        wgt->graph(currentGraph)->addData(round(x), round(y));
        wgt->replot();
        ++i;
    }
...
qint64 RasterAlgorithms::bresenhamLineAlgorithm(int x0, int y0, int x1, int y1)
{
    QElapsedTimer timer;
        timer.start();
...
qint64 RasterAlgorithms::bresenhamCircleAlgorithm(int x0, int y0, int rad)
{
    QElapsedTimer timer;
        timer.start();
    if (rad == 1)
...
qint64 RasterAlgorithms::castlePitwayAlgorithm(int x0, int y0, int x1, int y1)
{
    QElapsedTimer timer;
        timer.start();
    bool steep = abs(y1 - y0) > abs(x1 - x0);
